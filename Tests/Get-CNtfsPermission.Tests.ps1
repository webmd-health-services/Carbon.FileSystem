...
Describe 'Get-CNtfsPermission' {
    Context 'On Windows' -Skip:(-not $IsWindows) {
        BeforeEach {
            ...
            $script:containerPath = Join-Path (Get-CTempPath) $script:containerPath
    ...
    Context 'On Linux and macOS' -Skip:$IsWindows {
        ...
        It 'fails' {
            Get-CNtfsPermission -Path 'anyPath' -ErrorAction SilentlyContinue | Should -BeNullOrEmpty
            $Global:Error | Should -Match 'Get-CNtfsPermission function is only supported on Windows'
        }

        It 'can fail silently' {
            Get-CNtfsPermission -Path 'anyPath' -ErrorAction Ignore | Should -BeNullOrEmpty
